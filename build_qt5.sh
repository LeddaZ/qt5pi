...
function fetch_cross_compile_tool () {
    ...
    if [ ! -d "/src/gcc-linaro-7.5.0-2019.12-x86_64_arm-linux-gnueabihf" ]; then
        pushd /src/
        wget -q https://releases.linaro.org/components/toolchain/binaries/latest-7/arm-linux-gnueabihf/gcc-linaro-7.5.0-2019.12-x86_64_arm-linux-gnueabihf.tar.xz
...
function build_qt () {
    ...
    if [ ! -f "$BUILD_TARGET/qt5-$QT_BRANCH-$DEBIAN_VERSION-$1.tar.gz" ]; then
        ...
        /src/qt5/configure \
            ...
            -confirm-license \
            -dbus-linked \
            -device-option CROSS_COMPILE=/src/gcc-linaro-7.5.0-2019.12-x86_64_arm-linux-gnueabihf/bin/arm-linux-gnueabihf- \
            -eglfs \
            -evdev \
